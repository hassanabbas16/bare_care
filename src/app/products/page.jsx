    ...
    const fetchWishlist = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const productIds = data.wishlistItems.map((item) => item.product_id);
  ...
  const filterProducts = (categoryFromQuery, brandFromQuery) => {
    let filtered = products.filter((product) => {
      ...
      const productAuthenticity =
        product.authenticity !== undefined ? product.authenticity : true; // Assuming authentic by default
      ...
      const matchesSkinType =
        ...
        selectedSkinTypes.some((type) =>
          productSkinType.includes(type.toLowerCase())
        );
  ...
  return (
    <Box>
      ...
      >
        ...
        >
          {selectedBrandFromQuery
            ? `${selectedBrandFromQuery} Products`
            : `${category} Products`}
        </Typography>
      </Box>
          ...
          >
            ...
            {filteredProducts.length > 0 ? (
              <Grid
                container
                spacing={2}
                sx={{ padding: "0 2rem", justifyContent: "center", maxWidth: "1440px" }}
              >
                {filteredProducts.map((product) => (
                  <Grid item xs={12} sm={6} md={4} lg={3} key={product.id}>
                    <ProductCard
                      product={product}
                      isCompared={comparedProducts.some(
                        (p) => p.id === product.id
                      )}
